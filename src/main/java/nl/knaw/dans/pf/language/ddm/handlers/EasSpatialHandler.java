...
public class EasSpatialHandler extends CrosswalkHandler<EasyMetadata> {
    ...
    protected void initElement(final String uri, final String localName, final Attributes attributes) {
      ...
      if ("Polygon".equals(localName) && state == null)
        this.state = POLYGON;
          ...
          && (state == POLYGON || state == EXTERIOR || state == INTERIOR))
    ...
    private List<PolygonPoint> createPolygonPoints() throws SAXException {
      ...
      else if (!coordinates[0].equals(coordinates[length - 2])
          && !coordinates[1].equals(coordinates[length - 1])) {
...
/**
 * Parsing a polygon with the current parser is quite complex. A state machine helps with this.
 * Read these states from bottom to top as [state]([next-state])
 */
